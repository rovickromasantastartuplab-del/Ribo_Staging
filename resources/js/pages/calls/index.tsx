...
export default function Calls() {
  ...
  const handleFormSubmit = (formData: any) => {
    ...
    if (formMode === 'create') {
      ...
      router.post(route('calls.store'), formData, {
        ...
        onError: (errors) => {
          toast.dismiss();
          toast.error(t('Failed to create call: {{errors}}', { errors: Object.values(errors).join(', ') }));
        }
      });
      ...
      router.put(route('calls.update', currentItem.id), formData, {
        ...
        onError: (errors) => {
          toast.dismiss();
          toast.error(t('Failed to update call: {{errors}}', { errors: Object.values(errors).join(', ') }));
        }
      });
    }
  };
  ...
  const handleBulkAction = (action: string, selectedIds: any[]) => {
    if (action === 'bulk_delete') {
      if (!hasPermission(permissions, 'delete-calls')) {
      ...
      if (confirm(t('Are you sure you want to delete the selected {{count}} records? This action cannot be undone.', { count: selectedIds.length }))) {
        ...
        router.delete(route('calls.bulk-delete'), {
