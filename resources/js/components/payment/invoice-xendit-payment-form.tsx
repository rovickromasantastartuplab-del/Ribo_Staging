...
interface InvoiceXenditPaymentFormProps {
  invoiceId: number;
  amount: number;
  paymentType: string;
  xenditApiKey: string;
...
export function InvoiceXenditPaymentForm({
  invoiceId,
  amount,
  paymentType,
  xenditApiKey,
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      console.error('Xendit invoice payment error:', err);
  ...
  const formatPrice = (price: number) => {
    return window.appSettings?.formatCurrency(Number(price || 0)) || new Intl.NumberFormat('id-ID', {
      style: 'currency',
      currency: currency,
    }).format(price);
  };
