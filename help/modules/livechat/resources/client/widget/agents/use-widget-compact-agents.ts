...
export function useAgentWasActiveRecently(agentId: number | string): boolean {
  ...
  const agentIsOnline = useIsAgentOnline(agentId);
  return (
    agentIsOnline || agents.some(a => a.id === agentId && a.wasActiveRecently)
  );
}
...
function useIsAgentOnline(agentId: number | string): boolean {
  return !!useEchoStore(s =>
    s.presence[helpdeskChannel.name]?.find(
      user => user.isAgent && user.modelId === +agentId,
    ),
  );
}
