...
class TaxController extends Controller
{
        ...
        if ($tax) {
            try {
                if ($tax->products()->count() > 0) {
                    return redirect()->back()->with('error', __('Cannot delete tax that is currently assigned to one or more products.'));
                }
    ...
    public function bulkDelete(Request $request)
    {
        ...
        try {
            $taxes = \App\Models\Tax::whereIn('id', $validated['ids'])->where('created_by', createdBy())->get();
            
            if ($taxes->isEmpty()) {
                 return redirect()->back()->with('warning', __('No valid records selected to delete.'));
            }
            
            $inUse = $taxes->filter(fn($t) => $t->products()->count() > 0);
            $deletable = $taxes->filter(fn($t) => $t->products()->count() === 0);
            ...
            if ($inUse->isNotEmpty() && $deletable->isNotEmpty()) {
                ...
                return redirect()->back()->with('error', __('Cannot delete the selected taxes because they are currently assigned to products.'));
            }
